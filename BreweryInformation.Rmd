...
"beer" contains information regarding the beer data
"brew" contains information regardin the breweries data
```{r, echo=TRUE}
...
library(dplyr) #allows us to use the "count" function
count(brew, brew$State)
```
#Merge beer data with the breweries data.
```{r, echo=TRUE}
names(beer)[5]<- "Brew_ID" #making the merged columns the same
totalbeerinfo <- merge(beer, brew, by="Brew_ID") #brew ID is the only column that they have the same
